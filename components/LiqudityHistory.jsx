...
const GetLiquidity = ({setActiveComponent}) => {
  ...
  return <section id="price" className="pt-32">
            ...
            <div>
              <div className="bg-slate-950/40 rounded-xl hover:-translate-y-2 transition-all duration-500">
                <div className="border border-white/10 rounded-xl">
                  <div className="p-6">
                    <a className="flex items-center justify-center gap-2 border-white/10 text-white py-2 px-6 mt-6 rounded-lg hover:bg-primary-hover transition-all duration-300">
                      {pool.network}
                    </a>
                    <hr className="my-5 border-dashed border-white/10" />
                    <ul className="mt-3 text-sm text-default-700" role="list">
                      <li className="flex items-center gap-2 py-2">
                        <i onClick={() => navigator.clipboard.writeText(pool.token_A)} className="inline-block w-5 text-primary">
                          <GoCopy />
                        </i>
                      ...
                      <li className="flex items-center gap-2 py-2">
                        <i onClick={() => navigator.clipboard.writeText(pool.poolAddress)} className="inline-block w-5 text-primary">
                          ...
                          <span className="text-default-50">
                            Pool Address: {shortenAddress(pool.poolAddress)}
                          </span>
                      </li>
                    </ul>
                  </div>
